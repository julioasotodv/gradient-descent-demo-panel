...
input_loss_function = pn.widgets.input.TextAreaInput(name='Loss function',
                                                     placeholder=("Write here an expression for the loss in terms of w1 and " + 
                                                                  "w2. You can pass any numpy function (like np.whatever). " + 
                                                                  "For instance: np.log(np.abs(w2)+1) + 0.1 * np.abs(w1**3) - 0.5 * w1**2 + 1e-4 * w2 **2 * w1**2 + 12"),
                                                     value="np.log(np.abs(w2)+1) + 0.1 * np.abs(w1**3) - 0.5 * w1**2 + 1e-4 * w2 **2 * w1**2 + 12",
                                                     sizing_mode="stretch_width")
pane_beta_inicial_1_custom_loss = pn.widgets.FloatInput(name="Initial w₁:", 
                                                        sizing_mode="stretch_width",
                                                        value=-5.5,
                                                        height=50)
pane_beta_inicial_2_custom_loss = pn.widgets.FloatInput(name="Initial w₂:", 
                                                        sizing_mode="stretch_width",
                                                        value=4.0,
                                                        height=50)
w1_min = pn.widgets.FloatInput(name="w₁ axis (min):", 
                               sizing_mode="stretch_width",
                               value=-6.0,
                               height=50)
w1_max = pn.widgets.FloatInput(name="w₁ axis (max)", 
                               sizing_mode="stretch_width",
                               value=6.0,
                               height=50)
w2_min = pn.widgets.FloatInput(name="w₂ axis (min):", 
                               sizing_mode="stretch_width",
                               value=-6.0,
                               height=50)
w2_max = pn.widgets.FloatInput(name="w₂ axis (max):", 
                               sizing_mode="stretch_width",
                               value=6.0,
                               height=50)
...
generacion_loss_function = pn.widgets.Button(name="¡Generate!", sizing_mode="stretch_width")
...
pane_valor_loss = pn.pane.Markdown(object="Loss value: ", sizing_mode="stretch_width")
pane_vector_gradiente = pn.pane.Markdown(object="Gradient vector: ", sizing_mode="stretch_width")

slider_step_size = pn.widgets.DiscreteSlider(name='Step size (learning rate)', 
                                             options=[0.001, 0.01, 0.05, 0.1, 0.2, 0.5, 1.0, 5.0, 10.0, 80.0], 
                                             value=0.1, 
                                             sizing_mode="stretch_width")
boton_step = pn.widgets.Button(name="Compute gradient!",  
                               sizing_mode="stretch_width")
...
# Dashboard completo:
dashboard = pn.Column(pn.pane.HTML(object="<h1>Gradient Descent Demo</h1>",
                                   style={"text-align": "center",
                                          "margin-bottom": "0"},
                                   sizing_mode="stretch_width"),
                      pn.pane.HTML(object='<h3>Problem formulation (loss function)</h3>',
                                   sizing_mode="stretch_width",
                                   style={"margin-top": "0"}),
                      seleccion_loss_function,
                      grid_boton,
                      pn.pane.HTML(object='<br><h3>Algorithm</h3>',
                                   sizing_mode="stretch_width",
                                   style={"margin-top": "0"}),
                      row_plot,
                      pn.pane.HTML(object="""<p>Demo by <a href="https://github.com/julioasotodv">Julio Antonio Soto</a> 
                                             for IE University. Made with <a href="https://panel.holoviz.org/">Panel</a>, <a href="https://plotly.com/graphing-libraries/">Plotly</a>, 
                                             <a href="https://numpy.org/">numpy</a> and <a href="https://github.com/HIPS/autograd">autograd</a>.</p>"""),
                      sizing_mode="stretch_width"
                      )
...
def añadir_opciones_Customs_loss(*events):
    for event in events:
        if event.name=="value":
            if event.new == "Custom":
...
def pintar_elementos_gradient_descent(valores_betas):
    ...
    # Computar flecha del Gradient vector:
...
def iniciar_figura(click):
    ...
    if seleccion_loss_function.value == "Linear regression":
    ...
    elif seleccion_loss_function.value == "Multilayer perceptron":
    ...
    elif seleccion_loss_function.value == "Custom":
...
def actualizar_betas():
    ...
    pane_vector_gradiente.object = "Gradient vector: "
...
def gradient_descent_paso(event):
    if boton_step.name == "Update weights!":
        actualizar_betas()
        boton_step.name="Compute gradient!"
    else:
        try:
            actualizar_gradiente()
            boton_step.name="Update weights!"
        except ValueError:
            boton_step.name="¡Convergido!"
            boton_step.disabled=True

# Anclar callbacks a elementos:
seleccion_loss_function.param.watch(añadir_opciones_Customs_loss, ["value"], onlychanged=True)
